          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="p-4 rounded-xl bg-[var(--color-bg)] border border-[var(--color-border)]">
              <div className="flex justify-between items-start mb-1">
                ...
                <span className="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-bold rounded-full border bg-[var(--color-warning-bg)] text-[var(--color-warning)] border-[var(--color-warning)]/20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mb-6">
          ...
          <div>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between p-3.5 rounded-xl bg-[var(--color-down-bg)] border border-[var(--color-down)]/20">
                <div className="flex items-center gap-2">
                  <div className="w-6 h-6 rounded-md bg-[var(--color-down)]/15 flex items-center justify-center">
                    <svg className="w-3.5 h-3.5 text-[var(--color-down)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 15l7-7 7 7" /></svg>
                  </div>
                  <span className="text-sm font-medium text-[var(--color-text-primary)]">Resistance</span>
                </div>
                <span className="text-sm font-bold tabular-nums text-[var(--color-down)]">{data.resistance ? fmtIDR(data.resistance) : "-"}</span>
              </div>
              <div className="flex items-center justify-between p-3.5 rounded-xl bg-[var(--color-up-bg)] border border-[var(--color-up)]/20">
                <div className="flex items-center gap-2">
                  <div className="w-6 h-6 rounded-md bg-[var(--color-up)]/15 flex items-center justify-center">
                    <svg className="w-3.5 h-3.5 text-[var(--color-up)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" /></svg>
                  </div>
                  <span className="text-sm font-medium text-[var(--color-text-primary)]">Support</span>
                </div>
                <span className="text-sm font-bold tabular-nums text-[var(--color-up)]">{data.support ? fmtIDR(data.support) : "-"}</span>
              </div>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="rounded-xl border border-[var(--color-border)] overflow-hidden">
                {["100.0%", "61.8%", "50.0%", "38.2%", "0.0%"].map((ratio, i) => {
                  ...
                  return (
                    <div key={ratio} className={`flex items-center justify-between px-4 py-2.5 ${i > 0 ? "border-t border-[var(--color-border)]" : ""} ${isKey ? "bg-[var(--color-warning-bg)]/60" : "bg-[var(--color-bg)]"}`}>
                      <div className="flex items-center gap-2">
                        <div className="w-12 h-1.5 rounded-full bg-[var(--color-muted-bg)] overflow-hidden">
                          <div className="h-full rounded-full bg-[var(--color-warning)]" style={{ width: `${barWidth}%` }} />
                        </div>
                        <span className={`text-xs font-semibold ${isKey ? "text-[var(--color-warning)]" : "text-[var(--color-text-muted)]"}`}>{ratio}</span>
                      </div>

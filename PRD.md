...
| `accumulation` | object | Deteksi pola akumulasi post-ARA "siap terbang": `{valid, ready_to_fly, k_heavy, window_days, density_pct, rvol, max_rvol, ara_date, ara_ref_price, sma20, state_ma20, distance_pct, note, warning, reason}`. ARA (+`ACCUM_ARA_RISE_PCT`% harian) = puncak distribusi; baseline volume = rata-rata SELURUH hari post-ARA SEBELUM hari berjalan (tanpa hari ARA, tanpa hari ini); `ready_to_fly` true jika harga MASIH DI BAWAH level ARA (belum recovery), minimal `ACCUM_MIN_HEAVY_DAYS` hari dengan volume ≥ `ACCUM_HEAVY_RVOL` × baseline DAN kepadatan ≥ `ACCUM_DENSITY_PCT`% (default 30%) DAN close ≥ SMA(`ACCUM_MA20_DAYS`). `density_pct` (kepadatan hari heavy) adalah GATE wajib — tanpa density, edge mati (validasi: b10 8.7% ≈ kontrol). `state_ma20` = `above`/`breakout`/`below` |
